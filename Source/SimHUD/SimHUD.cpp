...
#include "Vehicles/Both/SimModeHeterogeneous.h"
...
void ASimHUD::BeginPlay()
{
    ...
    try {
        ...
        if (!simmode_) {
            UE_LOG(LogTemp, Error, TEXT("HET_MARKER_NULL: SimMode is null (spawn failed). "));
            return;
        }

        UE_LOG(LogTemp, Warning, TEXT("HET_MARKER: about to startApiServer"));
...
void ASimHUD::createSimMode()
{
    ...
    if (simmode_name == AirSimSettings::kSimModeTypeMultirotor)
        ...
        simmode_ = this->GetWorld()->SpawnActor<ASimModeWorldHeterogeneous>(FVector::ZeroVector,
            FRotator::ZeroRotator,
            simmode_spawn_params);
