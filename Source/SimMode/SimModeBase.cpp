...
void ASimModeBase::startApiServer()
{
    if (getSettings().enable_rpc) {
        ...
        // ✅ Guard anti-crash : si le SimMode ne fournit pas de serveur, on log et on sort
        if (!api_server_) {
            UAirBlueprintLib::LogMessageString(
                "Cannot start RpcLib Server",
                "createApiServer() returned nullptr (SimMode did not provide an API server).",
                LogDebugLevel::Failure
            );
            return;
        }
