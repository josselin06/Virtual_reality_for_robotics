        ...
        private:
            ...
            static void loadRCSetting(const std::string& simmode_name, const Settings& settings_json, RCSettings& rc_setting)
            {
                ...
                if (settings_json.getChild("RC", rc_json)) {
                    rc_setting.remote_control_id = rc_json.getInt("RemoteControlID",
                        (simmode_name == kSimModeTypeMultirotor || simmode_name == kSimModeTypeBoth) ? 0 : -1);
                ...
                const std::map<std::string, std::shared_ptr<SensorSetting>>& sensor_defaults)
            {
                ...
                if (simmode_name == kSimModeTypeMultirotor) {
                    ...
                    // Create a default UAV + a default UGV when Vehicles{} is missing

                    // Default UAV: SimpleFlight
                    auto uav_setting = std::unique_ptr<VehicleSetting>(
                        new VehicleSetting("Drone1", kVehicleTypeSimpleFlight)
                    );
                    uav_setting->rc.remote_control_id = 0;
                    uav_setting->sensors = sensor_defaults;
                    vehicles[uav_setting->vehicle_name] = std::move(uav_setting);

                    // Default UGV: CPHusky (skid vehicle)
                    auto ugv_setting = std::unique_ptr<VehicleSetting>(
                        new VehicleSetting("Rover1", kVehicleTypeCPHusky)
                    );
                    ugv_setting->sensors = sensor_defaults;
                    vehicles[ugv_setting->vehicle_name] = std::move(ugv_setting);
                }
                    ...
                    throw std::invalid_argument(Utils::stringf(
                        "HET_MARKER - Unknown SimMode: %s, failed to set default vehicle settings", simmode_name.c_str())
                        .c_str());
                }
            }
                ...
                std::map<std::string, std::shared_ptr<SensorSetting>>& sensors)
            {
                ...
                sensors["lidar"] = createSensorSetting(SensorBase::SensorType::Lidar,"lidar",true);
